...
Troi's playlists are generated by patches, which are small scripts that build a playlist
generation pipelines. To list available patches:
...
LB Radio creates global playlists, which contains MusicBrainz MBIDs and needs to be resolved
to a music collection before it can be played. To create such a playlist on easy mode with an
artist and a tag use the following:
