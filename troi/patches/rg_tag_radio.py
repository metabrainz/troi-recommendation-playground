...
class TaggedReleaseGroupsSourceElement(troi.Element):
    ...
    def read(self, entities):
        ...
        r = requests.post("https://datasets.listenbrainz.org/recording-from-rg-tag/json",
                          json=[{'[tag]': tag } for tag in self.tags])
        return [ Recording(mbid=recording["recording_mbid"]) for recording in r.json() ]


class ReleaseGroupTagRadioPatch(troi.patch.Patch):
    """
       Make a playlist from release_groups that have been tagged with the same tags/genres.
    """
    ...
    def inputs():
        """
        Generate a release_group tag/genre radio playlist from selected tags/genres.

        \b
        USER_NAME is a MusicBrainz user name that has an account on ListenBrainz.
        TAGS release group level tags to select recordings from.
        """
        return [{
            "type": "argument",
            "args": ["user_name"],
    ...
    @staticmethod
    def description():
        return "Generate with recordings from one or more given release group tags."
    ...
    def create(self, inputs):
        ...
        source = TaggedReleaseGroupsSourceElement(tags)
        ...
        tag_list = ", ".join(tags)
        pl_maker = PlaylistMakerElement(name="Release Group Tag radio for tags %s" % tag_list,
                                        desc="Randomly selected tracks whose release_groups have been tagged with '%s'" % tag_list,
