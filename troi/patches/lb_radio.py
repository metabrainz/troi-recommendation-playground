...
from troi.patches.lb_radio_classes.recording import LBRadioRecordRecordingElement
...
class LBRadioPatch(troi.patch.Patch):
    ...
    def lookup_artist(self, artist_name):
        ...
        err_msg = (
            f"Artist {artist_name} could not be looked up. Please use exact spelling."
        )
        ...
        while True:
            ...
            if r.status_code != 200:
                raise RuntimeError(
                    f"Could not resolve artist name {artist_name}. Error {r.status_code} {r.text}"
                )
    ...
    def lookup_artist_from_mbid(self, artist_mbid):
        ...
        while True:
            r = requests.get(
                f"https://musicbrainz.org/ws/2/artist/%s?fmt=json" % str(artist_mbid)
            )
            if r.status_code == 404:
                raise RuntimeError(
                    f"Could not resolve artist mbid {artist_mbid}. Error {r.status_code} {r.text}"
                )
            ...
            if r.status_code != 200:
                raise RuntimeError(
                    f"Could not resolve artist name {artist_mbid}. Error {r.status_code} {r.text}"
                )
    ...
    def create(self, inputs):
        ...
        for element in prompt_elements:
            ...
            if element["entity"] == "country":
                if isinstance(element["values"][0], UUID):
                    source = LBRadioCountryRecordingElement(
                        mode, area_mbid=element["values"][0]
                    )
                else:
                    source = LBRadioCountryRecordingElement(
                        mode, area_name=element["values"][0]
                    )
            ...
            if element["entity"] == "playlist":
                source = LBRadioPlaylistRecordingElement(
                    element["values"][0], mode=mode
                )
            ...
            if element["entity"] == "stats":
                ...
                source = LBRadioStatsRecordingElement(
                    element["values"][0], mode=mode, time_range=element["opts"][0]
                )
            ...
            if element["entity"] == "recording":
                if len(element["values"]) == 0:
                    raise RuntimeError("Recoding MBID cannot be blank")
                source = LBRadioRecordRecordingElement(element["values"][0], mode=mode)
        ...
        blend = WeighAndBlendRecordingsElement(
            weights, max_num_recordings=100, max_artist_occurrence=3
        )
        blend.set_sources(elements)

        pl_maker = PlaylistMakerElement(
            patch_slug=self.slug(), max_num_recordings=TARGET_NUMBER_OF_RECORDINGS
        )
    ...
    def post_process(self):
        ...
        desc = (
            "Experimental ListenBrainz radio using %s mode, which was generated from this prompt: '%s'"
            % (self.mode, prompt)
        )
